...
local CREATURE_TYPE_SUMMON_OWN = 3
creatures = {}
...
function init()  
  ...
  g_keyboard.bindKeyDown('Tab', function()
    return attackNextTarget(false) or false
  end)
...
function doCreatureFitFilters(creature)
  ...
  if localPlayer and creature == localPlayer then
    return false
  end
...
function attackNextTarget(force)
  local player = g_game.getLocalPlayer()
  if not player or modules.game_console.isChatEnabled() then
    return false
  end

  local playerPos = player:getPosition()

  local candidateCreatures = {}
  local mapPanel = modules.game_interface.getMapPanel()
  local dimension = mapPanel and mapPanel.getVisibleDimension and mapPanel:getVisibleDimension() or nil
  local halfX = (dimension and dimension.width and dimension.width > 0) and math.floor(dimension.width / 2) or 8
  local halfY = (dimension and dimension.height and dimension.height > 0) and math.floor(dimension.height / 2) or 8
  if halfX < 3 then halfX = 8 end
  if halfY < 3 then halfY = 8 end
  ...
  local maxCreatures = (battlePanel and battlePanel:getChildCount()) or 30
  for _, creature in ipairs(spectators) do
    if creature:isMonster()
      and creature:getType() ~= CREATURE_TYPE_SUMMON_OWN
      and creature ~= player
      and creature:getHealthPercent() > 0
      and creature:canBeSeen()
    then
      local pos = creature:getPosition()
      if pos and pos.z == playerPos.z then
        table.insert(candidateCreatures, creature)
        if #candidateCreatures >= maxCreatures then break end
      end
    end
  end

  local validCreatures = {}
  for _, creature in ipairs(candidateCreatures) do
    local isTargetValid = creature:isMonster()
      and creature:getType() ~= CREATURE_TYPE_SUMMON_OWN
      and creature:getHealthPercent() > 0
      and creature:canBeSeen()
      and creature ~= player
  ...
  for _, creature in ipairs(validCreatures) do
    if not targetSet[creature:getId()] then
      allTargeted = false
      break
    end
  end
  ...
  if not nextTarget then
    nextTarget = validCreatures[1]
  end

  local closestCreature = validCreatures[1]
  local closestDistance = getDistanceBetween(playerPos, closestCreature:getPosition())
  
  local bestTarget = nextTarget
  local bestHealth = nextTarget:getHealthPercent()
  ...
  for _, creature in ipairs(validCreatures) do
    if not targetSet[creature:getId()] then
      ...
      if distance <= closestDistance + 2 and health < bestHealth then
        bestTarget = creature
        bestHealth = health
      end
    end
  ...
  targetSet[bestTarget:getId()] = true
  g_game.attack(bestTarget)
  
  for i, creature in ipairs(candidateCreatures) do
    if creature:getId() == bestTarget:getId() then
      lastAttackedCreatureIndex = i
      break
    end
  end
