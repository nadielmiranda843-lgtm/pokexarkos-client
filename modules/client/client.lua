...
local gameMusicFilename = "/sounds/saffron"
local musicChannel = nil
local gameMusicEvent = nil

local function cancelGameMusicEvent()
  if gameMusicEvent then
    removeEvent(gameMusicEvent)
    gameMusicEvent = nil
  end
end

local function playLoginMusic()
  cancelGameMusicEvent()
  if musicChannel == nil then return end

  musicChannel:stop(1)
  musicChannel:enqueue(musicFilename, 1)
end
...
local function onMusicGameStart()
  ...
  musicChannel:stop(1)
  gameMusicEvent = scheduleEvent(function()
    gameMusicEvent = nil
    if g_game.isOnline() and musicChannel ~= nil then
      musicChannel:stop(1)
      musicChannel:enqueue(gameMusicFilename, 1, 0.35)
    end
  end, 10000)
end
...
function init()
  ...
  if g_sounds ~= nil then
    ...
    g_sounds.preload(gameMusicFilename)
  end
