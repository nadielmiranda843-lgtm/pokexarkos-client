local gameStart = 0
local suppressedContainerWindows = {}

local function normalizeContainerName(name)
...
local function consumeSuppressedContainerWindow(name)
  local key = normalizeContainerName(name)
  local count = suppressedContainerWindows[key]
  if not count or count <= 0 then
    return false
  end

  if count == 1 then
    suppressedContainerWindows[key] = nil
  else
    suppressedContainerWindows[key] = count - 1
  end
  return true
end

function suppressContainerWindowOnce(name)
  local key = normalizeContainerName(name)
  if key == '' then return end
  suppressedContainerWindows[key] = (suppressedContainerWindows[key] or 0) + 1
end
...
function onContainerOpen(container, previousContainer)
  if not previousContainer and consumeSuppressedContainerWindow(container:getName()) then
    container.window = nil
    container.itemsPanel = nil
    return
  end
