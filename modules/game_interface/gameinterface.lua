...
function tryExit()
  ...
  exitWindow = displayGeneralBox('Sair do PokeXarkos', "Voce esta prestes a fechar o PokeXarkos.\n\nUse 'Sair com seguranca' para desconectar seu personagem corretamente.\nUse 'Forcar saida' somente se quiser fechar o cliente agora, pois seu personagem pode continuar online por alguns instantes.",
  { { text='Forcar saida', callback=exitFunc },
    { text='Sair com seguranca', callback=logoutFunc },
...
function onMouseGrabberRelease(self, mousePosition, mouseButton)
  ...
  if mouseButton == MouseLeftButton then
    ...
    if clickedWidget then
      ...
      elseif selectedType == 'pokedex' then
        onPokedexSelect(clickedWidget, mousePosition)
      end
    end
  end
...
-- Cursor em modo "pokedex": clique em um alvo (monster/pokemon) para pedir as infos ao servidor
function onPokedexSelect(clickedWidget, mousePosition)
  ...
  if clickedWidget:getClassName() == 'UIGameMap' then
    local tile = clickedWidget:getTile(mousePosition)
    if tile then
      creature = tile:getTopCreatureEx(clickedWidget:getPositionOffset(mousePosition))
    end
  elseif clickedWidget:getClassName() == 'UICreatureButton' then
    creature = clickedWidget:getCreature()
  end
  ...
  -- mesmo opcode usado pelo servidor para responder a janela
  local OPCODE_POKEDEX = 107
  protocolGame:sendExtendedJSONOpcode(OPCODE_POKEDEX, {
    action = 'requestInfo',
    targetId = creature:getId(),
    monsterName = creature:getName()
  })
end
...
-- Abre o modo de seleção para Pokédex (sem precisar usar item)
function startPokedexSelect()
  gameMapPanel:blockNextMouseRelease()

  -- selectedThing precisa ser diferente de nil para o onMouseGrabberRelease não abortar
  selectedType = 'pokedex'
  selectedThing = true
  mouseGrabberWidget:grabMouse()
  g_mouse.pushCursor('target')
end
...
local function openOwnProfileFromLook(thing)
  if not thing or not thing.isCreature or not thing:isCreature() then
    return false
  end
  ...
  if modules and modules.game_profile then
    profileModule = modules.game_profile
  elseif mods and mods.game_profile then
    profileModule = mods.game_profile
  end
  ...
  if thing.isLocalPlayer and thing:isLocalPlayer() then
    isOwnPlayer = true
  elseif thing.getId and localPlayer.getId and thing:getId() == localPlayer:getId() then
    isOwnPlayer = true
  end

  if isOwnPlayer then
    if profileModule.openForCreature then
      profileModule.openForCreature(localPlayer)
      return true
    end

    if profileModule.toggle then
      profileModule.toggle()
      return true
    end
  end

  if thing.isPlayer and thing:isPlayer() and profileModule.openInspectForPlayer then
    if profileModule.openInspectForPlayer(thing) ~= false then
      return true
    end
  end
...
local function lookThingOrOpenProfile(thing)
  if not thing then
    return
  end

  if openOwnProfileFromLook(thing) then
    return
  end
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if lookThing then
    menu:addOption(tr('Look'), function() lookThingOrOpenProfile(lookThing) end, shortcut)
  end
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature, marking)
  ...
  if g_app.isMobile() then
    ...
    if action == "look" then
      if lookThing then
        resetLeftActions()
        lookThingOrOpenProfile(lookThing)
        return true    
      end
      return true    
