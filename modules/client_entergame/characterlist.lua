...
local function tryLogin(charInfo, tries)
  ...
  loadBox = displayCancelBox('Aguarde', 'Entrando no mundo PokeXarkos...')
...
local function updateWait(timeStart, timeEnd)
  if waitingWindow then
    ...
    if time <= timeEnd then
      ...
      label:setText('Tentando reconectar em ' .. timeStr .. ' segundos.')
...
function onGameLoginError(message)
  CharacterList.destroyLoadBox()
  errorBox = displayErrorBox('Erro de Login', message)
...
function onGameLoginToken(unknown)
  ...
  errorBox = displayErrorBox('Autenticacao em Duas Etapas', 'Um novo token de autenticacao e necessario.\nEntre novamente na sua conta.')
  errorBox.onOk = function()
    errorBox = nil
    EnterGame.show()
  end
...
function onGameConnectionError(message, code)
  ...
  if (not g_game.isOnline() or code ~= 2) and not errorBox then -- code 2 is normal disconnect, end of file
    ...
    errorBox = displayErrorBox('Erro de Conexao', text)
    errorBox.onOk = function()
      errorBox = nil
      CharacterList.showAgain()
    end
...
function onGameUpdateNeeded(signature)
  CharacterList.destroyLoadBox()
  errorBox = displayErrorBox('Atualizacao necessaria', 'Entre novamente com sua conta para atualizar o cliente.')
  errorBox.onOk = function()
    errorBox = nil
    CharacterList.showAgain()
  end  
