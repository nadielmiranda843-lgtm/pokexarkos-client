...
local function sanitizePassword(value)
  value = tostring(value or '')
  value = value:gsub('[\r\n]', '')
  return value
end
...
function EnterGame.doLogin(account, password, token, host)
  if g_game.isOnline() then
    local errorBox = displayErrorBox('Erro de Login', 'Nao e possivel entrar enquanto voce ja esta dentro do jogo.')
    connect(errorBox, { onOk = EnterGame.show })
    return
  end
  
  G.account = sanitizeAccountName(account or enterGame:getChildById('accountNameTextEdit'):getText())
  G.password = sanitizePassword(password or enterGame:getChildById('accountPasswordTextEdit'):getText())
  ...
  EnterGame.hide()
  loadBox = displayCancelBox('Aguarde', 'Conectando ao servidor de login do PokeXarkos...')
...
function EnterGame.onError(err)
  ...
  local errorBox = displayErrorBox('Erro de Login', err)
